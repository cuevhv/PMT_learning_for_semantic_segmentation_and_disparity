...
def read(file):
    ...
    elif file.endswith('.pgm'): return readpgm(file)
    elif file.endswith('.png'): return readpng(file)
    elif file.endswith('.jpg'): return readImage(file)
    elif file.endswith('.bin'): return readbin(file)
...
def readbin(file):
    x = np.fromfile(file, dtype='>f4', sep='')
    return np.reshape(x, [480, 640], order='F')
...
def readpgm(name):
    ...
    img = (np.array(data[3:]),(data[1],data[0]),data[2])
    
    return np.reshape(img[0],img[1])
