...
class ASPP(nn.Module):
    def __init__(self, in_channels=2048, out_channels=256, output_stride=8):
        ...
        # dilations = [6, 12, 18]
        if self.small_net:
            #dont forget to change the names in forward
            self.reduce_input1 = nn.Conv2d(in_channels, 256, 1, bias=False)
            in_channels = 256

            self.aspp0_0 = nn.Sequential(OrderedDict([('conv', nn.Conv2d(in_channels, out_channels, 1, bias=False)),
                                                    ('bn', nn.BatchNorm2d(out_channels)),
                                                    ('relu', nn.ReLU(inplace=True))]))
            self.aspp1_0 = SeparableConv2d(in_channels, out_channels, dilation=dilations[0], relu_first=False)
            self.aspp2_0 = SeparableConv2d(in_channels, out_channels, dilation=dilations[1], relu_first=False)
            self.aspp3_0 = SeparableConv2d(in_channels, out_channels, dilation=dilations[2], relu_first=False)

            self.image_pooling_0 = nn.Sequential(OrderedDict([('gap', nn.AdaptiveAvgPool2d((1, 1))),
...
class SPPDecoder(nn.Module):
    def __init__(self, in_channels, sep_channel=256, concat_prev=False, reduced_layer_num=48):
        super().__init__()
        self.concat_prev = concat_prev
        inplane_int_feat = 0
        if self.concat_prev:
            inplane_int_feat = 64
            self.conv_int_feat = nn.Conv2d(self.concat_prev, inplane_int_feat, 1, bias=False)

        self.conv = nn.Conv2d(in_channels, reduced_layer_num, 1, bias=False)
        self.bn = nn.BatchNorm2d(reduced_layer_num)
        self.relu = nn.ReLU(inplace=True)
        self.sep1 = SeparableConv2d(sep_channel+reduced_layer_num+inplane_int_feat, 256, relu_first=False)
        self.sep2 = SeparableConv2d(256, 256, relu_first=False)

    def forward(self, x, low_level_feat, other_feat=None):
        ...
        middle_feat = torch.cat((x, low_level_feat), dim=1)
        if self.concat_prev:
            middle_feat = torch.cat((middle_feat, self.conv_int_feat(other_feat)),
                                        dim=1)
        x = self.sep1(middle_feat)
        x = self.sep2(x)
        return x, middle_feat


def create_spp(dec_type, in_channels=2048, middle_channels=256, sep_channel=256, concat_prev=False, output_stride=8):
    print(dec_type)
    if dec_type == 'spp':
        return SPP(in_channels, middle_channels), SPPDecoder(middle_channels, sep_channel, concat_prev)
    elif dec_type == 'aspp':
        return ASPP(in_channels, middle_channels, output_stride), SPPDecoder(middle_channels, sep_channel, concat_prev)
