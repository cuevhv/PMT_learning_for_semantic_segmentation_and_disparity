...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid'):
    ...
    if activation_output:
        out = Conv2DTranspose(filters=1, kernel_size=3, strides=2, padding='same', name="disp_out", activation=activation_output)(a_b0)
    else:
        out = Conv2DTranspose(filters=1, kernel_size=3, strides=2, padding='same', name="disp_out")(a_b0)
    ...
    seg_branch = Concatenate()([a_b4, seg_branch])
    seg_branch = Conv2DTranspose(filters=512, kernel_size=3, strides=2, padding='same')(seg_branch)
    seg_branch = BatchNormalization(axis=bn_axis)(seg_branch)
    seg_branch = Activation('relu')(seg_branch)
    
    seg_branch = Concatenate()([a_b3, seg_branch])
    seg_branch = Conv2DTranspose(filters=256, kernel_size=3, strides=2, padding='same')(seg_branch)
    seg_branch = BatchNormalization(axis=bn_axis)(seg_branch)
    seg_branch = Activation('relu')(seg_branch)

    seg_branch = Concatenate()([a_b2, seg_branch])
    seg_branch = Conv2DTranspose(filters=256, kernel_size=3, strides=2, padding='same')(seg_branch)
    seg_branch = BatchNormalization(axis=bn_axis)(seg_branch)
    seg_branch = Activation('relu')(seg_branch)
    ...
    model.compile(optimizer=opt, loss={'disp_out': 'mse','seg_out': 'categorical_crossentropy'},
                                metrics={'disp_out': 'accuracy','seg_out': 'accuracy'})
    return model
