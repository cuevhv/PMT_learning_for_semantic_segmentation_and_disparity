...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid'):
    bn_axis = __get_normalization_axis()
    mask_shape = [input_shape[0], input_shape[1], labels]
    ...
    out = Lambda(lambda x: x * 10)(out)
    
    #out = tf.keras.backend.permute_dimensions(tf.keras.backend.squeeze(tf.keras.backend.stack([out for i in range(labels)]), axis=-1), [1,2,3,0])
    #out = tf.keras.backend.squeeze(tf.keras.backend.stack([out for i in range(labels)]), axis=-1)
    out_list = []
    mask_names = []
    for i in range(labels):
        name = 'disp_out_'+str(i)
        mask_names.append(name)
        out_m = Multiply(name='disp_out_'+str(i))([out, tf.expand_dims(mask[:,:,:,i], axis=-1)])
        out_list.append(out_m)

    out_disp = Lambda(lambda x: tf.concat(out_list, axis=3), name='disp_out')(out_list)
    out_list = [out_disp]
    mask_names = ['disp_out']
    # for i in range(labels):
    #     name = 'disp_out'+str(i)
    #     mask_names.append(name)
    #     masked_out.append(tf.math.multiply(out, tf.expand_dims(mask[:,:,:,i], axis=-1), name=name))

    # print 'branches out: ', out.shape    
    ...
    model = Model(inputs = [input_a, input_b, mask], outputs=out_list+[seg_branch])
    model.summary()
    ...
    losses = {'seg_out': 'categorical_crossentropy'}
    metrics = {'seg_out': 'accuracy'}

    for name in mask_names:
        losses[name] = multiple_huber_loss(labels)
        metrics[name] = pixel_mae


    model.compile(optimizer=opt, experimental_run_tf_function=False, loss=losses,
                                metrics=metrics)
    return model
