...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid', weights_path='', strategy=''):
    ...
    #with strategy.scope():    
        ...
        x = UpSampling2D(2)(x)
        x = Conv2D(filters=64, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(x)
        ...
        seg_branch = UpSampling2DBilinear([input_a.shape[1], input_a.shape[2]])(seg_branch)
        #raw_input()
        ...
        y = Conv2D(filters=128, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(y)
        ...
        disp_out = Conv2D(filters=64, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(disp_out)
        ...
        x = Conv2D(filters=32, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(x)
        x = conv2DT_BA(x, filters=32, kernel = 3, strides=2, padding='same')
        ...
        seg_branch2 = UpSampling2DBilinear([input_a.shape[1], input_a.shape[2]])(seg_branch2)
        #raw_input()
        ...
        y4 = Conv2D(filters=128, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(y4)
        y = UpSampling2D(4)(y)
        ...
        disp_out2 = Conv2D(filters=64, kernel_size=1, kernel_initializer=tf.initializers.he_normal())(disp_out2)
        ...
        losses = {'disp_out': masked_MAE,
                    ...
                    'seg_out2': 'categorical_crossentropy'}
