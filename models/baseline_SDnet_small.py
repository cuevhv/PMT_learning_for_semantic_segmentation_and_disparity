...
def Getcorr1d(x, y, kernel_size = 3, displacement = 1, stride_1 = 1, stride_2 = 1, padding = 0):
    ...
    out = CorrelationCost(kernel_size, displacement, stride_1, stride_2, padding, data_format='channels_last')([input_a, input_b])
...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid', weights_path='', strategy=''):
    ...
    #with strategy.scope():    
        ...
        x = Conv2D(filters=32, kernel_size=1)(x)
        x = conv2DT_BA(x, filters=32, kernel = 3, strides=2, padding='same')
        ...
        y4 = Conv2D(filters=128, kernel_size=1)(y4)
        y = UpSampling2D(4)(y)
        y = Concatenate()([y4, y])
        ...
        disp_out2 = Conv2D(filters=64, kernel_size=1)(disp_out2)
        ...
        opt = 'adam'
        opt = optimizers.Adam(lr=0.01, decay=0.01)#lr=0.001

    
        losses = {'disp_out': dynamic_huber_loss,
                    'disp_out2': dynamic_huber_loss,
