...
def conv2D_block(feature1, feature2, filter_size):
    ...
    conv2d_output = Activation('relu')(conv2d_output)
    return BatchNormalization(axis=bn_axis)(conv2d_output)
#----------------------------------------------------------------------------------------------------------------------------------------------------
...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid'):
    ...
    branches_a[1] = BatchNormalization(axis=-1)(branches_a[1])
    ...
    branches_a[0] = Conv2D(32,1)(branches_a[0])
    branches_a[0] = BatchNormalization(axis=-1)(branches_a[0])
    ...
    out_a = Conv2D(128, 5)(a_b0)
    out = Conv2D(64, 5)(a_b0)
    out = Conv2DTranspose(filters=128, kernel_size=5, strides=1, padding='same')(out)
    out = Add()([out_a, out])
    out = Conv2D(64, 3)(out)
    ...
    model.compile(optimizer=opt, loss={'disp_out': dynamic_huber_loss, 'seg_out': 'categorical_crossentropy'},
                                metrics={'disp_out': 'mae','seg_out': 'accuracy'})
    return model
