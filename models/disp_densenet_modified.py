...
from tensorflow.keras.layers import Dense, Dropout, Lambda, Multiply
...
def loadModel(backboneName = 'resnet50', labels=5, input_shape=(224, 224, 3), activation_output='sigmoid'):
    ...
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = Conv2D(filters=64, kernel_size=1, padding='same')(x)
    x = UpSampling2D(8)(x)
    ...
    y = BatchNormalization()(x)
    y = Activation('relu')(y)
    y = Conv2D(filters=64, kernel_size=5, padding='same')(y)

    x = concatenate([x, y])
    y = BatchNormalization()(x)
    y = Activation('relu')(y)
    max_disp = 128
    y = Conv2DTranspose(filters=max_disp, kernel_size=9, padding='same')(y)
    alpha = Activation('softmax')(-y)
    #disp = tf.range(64, dtype=tf.float32)
    disp = tf.range(0, 1+1/float(max_disp - 1), 1/float(max_disp - 1), dtype=tf.float32)

    yout = alpha * disp
    yout = tf.reduce_sum(yout, axis=-1, keepdims=True)
    if activation_output:
        yout = Activation(None, name='disp_out')(yout)
    else:
        yout = Activation(None, name='disp_out')(yout)
    # prediction
    ...
    # else:
    model.compile(loss={'disp_out': area_loss},
                optimizer=RMSprop(lr=lr),
