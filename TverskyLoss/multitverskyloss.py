...
class MultiTverskyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for idx in range(num_class):
            ...
            # loss_func = FocalBinaryTverskyLoss(self.alpha, self.beta, self.gamma).apply
            # loss_idx = loss_func(input_idx, target_idx)
            loss_func = FocalBinaryTverskyLoss().apply
            loss_idx = loss_func(input_idx, target_idx)
            # print(idx, targets.shape, input_idx.shape, loss_idx)
